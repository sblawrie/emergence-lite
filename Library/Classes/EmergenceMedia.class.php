...
class EmergenceMedia extends EmergenceModel
{

	// support subclassing
	static public $rootClass = __CLASS__;
	static public $defaultClass = __CLASS__;
	static public $subClasses = array(__CLASS__,'PhotoMedia','AudioMedia','VideoMedia','PDFMedia','FlashMedia','ExternalMedia');
	...
	static public $fields = array(
		...
		,'MIMEType' => array(
			'type' => 'enum'
			,'values' => array('image/gif','image/jpeg','image/png','video/x-flv','application/pdf','application/x-shockwave-flash','text/plain')
		)
		,'Width' => array(
			'type' => 'integer'
			,'unsigned' => true
	...
	static public $newDirectoryPermissions = 0755;
	...
	static public $mediaPath = '/var/images';
	...
	function __get($name)
	{
		switch($name)
		{
			...
			case 'FilesystemPath':
				...
				return static::$mediaPath.'/media/original/'.$this->Filename;
...
*/
	...
	*/
	...
	static public function createFromFile($file, $fieldValues = array())
	{
		...
		if(!is_writable(static::$mediaPath))
		{
			throw new Exception(static::$mediaPath . ' is not writable.');
		}
		...
		if (!$success)
		{
			...
			if(!file_exists(dirname($Media->FilesystemPath))) {
			
				if(!mkdir(dirname($Media->FilesystemPath), static::$newDirectoryPermissions, true))
				{
					throw new Exception('Failed to create directory. ' . $Media->FilesystemPath . ' ' . static::$newDirectoryPermissions);
				}
	...
	static public function analyzeFile($filename)
	{
		...
		// hash file
		$mediaInfo['SHA1'] = sha1_file($filename);
