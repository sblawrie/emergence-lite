...
class Dwoo_Plugin_extends extends Dwoo_Plugin implements Dwoo_ICompilable
{
	...
	public static function compile(Dwoo_Compiler $compiler, $file)
	{
		...
		while (!empty($file)) {
			...
			/*if(strrpos($file,':') !== false && strrpos($file,'?') !== false)
			{
				$tmp['file'] = $file;
				...
				$tmp['first'] = substr($file,0,$tmp['?']);
				$tmp['middle'] = substr($file,$tmp['?'],$tmp[':'] - $tmp['?']);
				$tmp['last'] = substr($file,$tmp[':']+1);
			...
			if ($file === '""' || $file === "''" || (substr($file, 0, 1) !== '"' && substr($file, 0, 1) !== '\'')) {
				throw new Dwoo_Compilation_Exception($compiler, 'Extends : The file name must be a non-empty string. ' . $file);
				return;
			}
